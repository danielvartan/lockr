test_that("test_private_key | general test", {
    expect_false(test_private_key("base"))

    mock <- function(.parent = parent.frame(), .env = topenv(.parent)) {
        mockr::with_mock(
            file.exists_ = function(...) TRUE,
            test_private_key("base")
        )
    }
...
test_that("assert_private_key | general test", {
    expect_error(assert_private_key("base"))

    mock <- function(.parent = parent.frame(), .env = topenv(.parent)) {
        mockr::with_mock(
            test_private_key = function(...) TRUE,
            assert_private_key("base")
        )
    }
...
test_that("test_public_key | general test", {
    expect_false(test_public_key("base"))

    mock <- function(.parent = parent.frame(), .env = topenv(.parent)) {
        mockr::with_mock(
            file.exists_ = function(...) TRUE,
            test_public_key("base")
        )
    }
...
test_that("assert_public_key | general test", {
    expect_error(assert_public_key("base"))

    mock <- function(.parent = parent.frame(), .env = topenv(.parent)) {
        mockr::with_mock(
            test_public_key = function(...) TRUE,
            assert_public_key("base")
        )
    }
